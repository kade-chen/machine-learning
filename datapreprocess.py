...
def PreprocessData():
    ...
    # 按 dayofweek 分组，计算 trip_total 的总和，并在第一个子图 ax[0] 绘制柱状图
    df[["dayofweek", "trip_total"]].groupby("dayofweek").trip_total.sum().plot(
        kind="bar", ax=ax[0]
    )
    ax[0].set_title("Sum of trip_total")  # 设置第一个子图的标题

    # 按 dayofweek 分组，计算 trip_total 的平均值，并在第二个子图 ax[1] 绘制柱状图
    df[["dayofweek", "trip_total"]].groupby("dayofweek").trip_total.mean().plot(
        kind="bar", ax=ax[1]
    )
    ax[1].set_title("Avg. of trip_total")  # 设置第二个子图的标题
    plt.show()
    ...
    # 按小时分组，计算 trip_total 的总和，在第一个子图绘制柱状图
    df[["hour", "trip_total"]].groupby("hour").trip_total.sum().plot(kind="bar", ax=ax[0])
    ax[0].set_title("Sum of trip_total")  # 设置第一个子图标题

    # 按小时分组，计算 trip_total 的平均值，在第二个子图绘制柱状图
    df[["hour", "trip_total"]].groupby("hour").trip_total.mean().plot(kind="bar", ax=ax[1])
    ax[1].set_title("Avg. of trip_total")  # 设置第二个子图标题
